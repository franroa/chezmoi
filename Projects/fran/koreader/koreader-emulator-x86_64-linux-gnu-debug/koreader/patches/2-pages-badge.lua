...
local page_text_color = "auto"                   -- Choose your desired color or "auto" for UI setting
...
local background_color = "auto"                  -- Choose your desired color or "auto" for UI setting
    ...
    function MosaicMenuItem:paintTo(bb, x, y)
        ...
        if not self.is_directory and not self.file_deleted and self.status ~= "complete" and not self.been_opened then
            ...
            if page_count then
                ...
                local actual_text_color = page_text_color
                if page_text_color == "auto" then
                    actual_text_color = Blitbuffer.colorFromString(G_reader_settings:readSetting("ui_font_color_hex", "#000000"))
                end
                local pages_text = TextWidget:new({
                    text = page_text,
                    face = Font:getFace("cfont", font_size),
                    alignment = "left",
                    fgcolor = actual_text_color,
                    bold = true,
                    padding = 2,
                })

                local actual_bg_color = background_color
                if background_color == "auto" then
                    actual_bg_color = Blitbuffer.colorFromString(G_reader_settings:readSetting("ui_background_color_hex", "#ffffff"))
                end
