...
function cleanupTempFiles() {
    try {
        unlinkSync(ORIG_FILE);
    }
    catch { }
    try {
        unlinkSync(PROP_FILE);
    }
    catch { }
}
// ── Tool name constants ───────────────────────────────────────────
    ...
    return {
        "tool.execute.before": async (input, output) => {
            ...
            if (tool === "bash") {
                ...
                nvimSend(socket, `vim.defer_fn(function() pcall(function() require('claude-preview.neo_tree').reveal('${escapeLua(rmPaths[0])}') end) end, 300)`);
                return;
            }
            // ── Skip non-edit tools ───────────────────────────────
            ...
            switch (tool) {
                case "edit": {
                    ...
                    proposed = applyEdit(original, args.oldString ?? "", args.newString ?? "", args.replaceAll ?? false);
                    break;
                }
                case "write": {
                    filePath = args.filePath;
            ...
            if (changeStatus === "modified") {
                nvimSend(socket, `vim.defer_fn(function() pcall(function() require('claude-preview.neo_tree').reveal('${filePathEsc}') end) end, 300)`);
            }
            else {
                ...
                nvimSend(socket, `vim.defer_fn(function() pcall(function() require('claude-preview.neo_tree').reveal('${escapeLua(revealDir)}') end) end, 300)`);
            }
            // Show diff
            nvimSend(socket, `require('claude-preview.diff').show_diff('${origEsc}', '${propEsc}', '${displayEsc}')`);
        },
        "tool.execute.after": async (input, _output) => {
            ...
            if (tool === "bash") {
                ...
                nvimSend(socket, "vim.defer_fn(function() pcall(function() require('claude-preview.neo_tree').refresh() end) end, 200)");
                return;
            }
            if (!EDIT_TOOLS.has(tool))
                return;
            ...
            if (filePath) {
                const filePathEsc = escapeLua(filePath);
                nvimSend(socket, `vim.defer_fn(function() pcall(function() require('claude-preview.neo_tree').refresh() end) vim.defer_fn(function() pcall(function() require('claude-preview.neo_tree').reveal('${filePathEsc}') end) end, 200) end, 200)`);
            }
            else {
                nvimSend(socket, "vim.defer_fn(function() pcall(function() require('claude-preview.neo_tree').refresh() end) end, 200)");
            }
