 ...
 */
export function findNvimSocket(projectCwd) {
    const logFile = "/tmp/nvim-plugin-debug.log";
    const log = (msg) => writeFileSync(logFile, `[${new Date().toISOString()}] ${msg}\n`, { flag: "a" });
    log(`findNvimSocket called, projectCwd: ${projectCwd}`);
    log(`NVIM_LISTEN_ADDRESS: ${process.env.NVIM_LISTEN_ADDRESS}`);
    // 1. Check explicit env var
    const envSocket = process.env.NVIM_LISTEN_ADDRESS;
    if (envSocket && isSocketAlive(envSocket)) {
        log(`Found socket via env: ${envSocket}`);
        return envSocket;
    }
    // 2. Scan known socket locations
    const liveSockets = [];
    log("Scanning for sockets...");
    ...
    // Linux: /tmp/nvim.*/0
    log("Scanning Linux /tmp...");
    try {
        const result = execSync("ls /tmp/nvim.*/0 2>/dev/null", {
            encoding: "utf-8",
            timeout: 3000,
        }).trim();
        log(`Linux scan result: "${result}"`);
        for (const line of result.split("\n").filter(Boolean)) {
    ...
    log(`Found ${liveSockets.length} live sockets`);
    if (liveSockets.length === 0) {
        log("No sockets found, returning null");
        return null;
    }
...
export function escapeLua(str) {
    return str.replace(/\\/g, "\\\\").replace(/'/g, "\\'");
}
/** Send a Lua command to Neovim via --remote-send. */
export function nvimSend(socket, luaCmd) {
    try {
        execSync(`nvim --server "${socket}" --remote-send ":lua ${luaCmd}<CR>"`, { timeout: 5000, stdio: "ignore" });
        return true;
    }
    catch {
        return false;
