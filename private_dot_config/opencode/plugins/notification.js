...
function findTerminalWindow() {
  ...
  // Check if we're running inside any tmux session (with or without $TMUX set).
  // This handles headless aoe sessions where $TMUX is not propagated to child
  // processes (opencode is spawned by the tmux server, not a tmux client).
  const sessionProc = Bun.spawnSync(["tmux", "display-message", "-p", "#{session_name}"]);
