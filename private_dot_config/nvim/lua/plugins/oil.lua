...
--           vim.cmd.ed(join(preview.window_path, "/"))
--         end)
--       elseif vim.fn.isdirectory(join(preview.window_path, "/")) then
--         vim.api.nvim_win_call(preview.window_number, function()
--           oil.open(join(preview.window_path, "/"))
--         end)
--       end
--     else
--       vim.api.nvim_win_call(preview.window_number, function()
...
--   local current_dir = oil.get_current_dir()
--   local selected_file = oil.get_cursor_entry()
--   vim.notify(tostring(current_dir))
--   vim.notify(selected_file)
--
...
-- require("functions/helper_funcs")
-- return {
--   "oil.nvim",
--   dir = local_plugins_path .. "oil.nvim",
--   lazy = false,
--   config = function()
...
-- 1. Create a descriptive augroup
local my_augroup = vim.api.nvim_create_augroup("MyBufReadEvents", { clear = true })
    ...
    if vim.bo.filetype == "oil" then
      ...
      vim.schedule(function()
        ...
        for _, win_id in ipairs(all_win_ids) do
          ...
          if win and win.view.get_title() == "Oil Explorer" then
...
return {
  {
    ...
    opts = {
      ...
      keymaps = {
        ...
        -- ["<CR>"] = { "actions.select", opts = { tab = true, close = false } },
        ["<C-r>"] = '<cmd>lua require("oil").select({close=false, tab=true})<cr>',
      },
    },
    -- dir = local_plugins_path .. "oil.nvim",
  },
  ...
  },
  {
    ...
    dependencies = { "stevearc/oil.nvim" },
    enabled = function()
      local function is_oil_remote()
        local filepath = vim.fn.expand("%")
        if filepath:find("oil-ssh://", 1, true) then
          return true
        else
          return false
        end
      end
      return not is_oil_remote()
    end,
    opts = {},
  },
  {
