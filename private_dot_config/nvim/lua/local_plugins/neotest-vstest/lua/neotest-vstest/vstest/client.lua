...
---Escape an argument by wrapping it in quotes if it contains spaces or special characters
---@param arg string
---@return string
...
---@param additional_args? string[] additional arguments to pass to the test runner
---@return string process_output_path, string result_stream_file_path, string result_file_path
function M.run_tests(runner, ids, additional_args)
   ...
   if additional_args and #additional_args > 0 then
     for _, arg in ipairs(additional_args) do
       table.insert(command_parts, escape_arg(arg))
     end
   end
...
function M.debug_tests(runner, ids, additional_args)
   local process_output_path = nio.fn.tempname()
   lib.files.write(process_output_path, "")
