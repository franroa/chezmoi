...
function Client:run_tests(ids, additional_args)
   local nodes = {}
   ...
   return mtp_client.run_tests(self.project.dll_file, nodes, additional_args)
end
...
---@param additional_args? string[] additional arguments to pass to the test runner
---@return neotest-vstest.Client.RunResult
function Client:debug_tests(ids, additional_args)
   local nodes = {}
   for _, node in ipairs(self.test_nodes) do
     if vim.tbl_contains(ids, node.uid) then
       nodes[#nodes + 1] = sanitize_node(node)
   ...
   return mtp_client.debug_tests(self.project.dll_file, nodes, additional_args)
end
