...
# Function to get neovim project root from tracked file or active window or any running nvim
get_nvim_root() {
	# First: try tracked nvim project from tmux/neovim hooks
	if [[ -f /tmp/last_nvim_root ]]; then
		local tracked_root
		tracked_root=$(cat /tmp/last_nvim_root 2>/dev/null)
		# Accept if directory exists (don't require .git for tracked paths)
		if [[ -n "$tracked_root" && -d "$tracked_root" ]]; then
			echo "$tracked_root"
			return 0
		fi
	fi

	# Second: check active window
	local active_window
...
if [[ "$SCRATCHPAD" == "aoe" ]]; then
	...
	# Fallback CWD when no nvim window is focused or no nvim is running:
	# try active window's process CWD, then $HOME
	if [[ -z "$NVIM_CWD" ]]; then
		ACTIVE_PID=$(hyprctl activewindow -j 2>/dev/null | jq -r '.pid // empty' 2>/dev/null)
		if [[ -n "$ACTIVE_PID" ]]; then
			PROC_CWD=$(readlink -f "/proc/${ACTIVE_PID}/cwd" 2>/dev/null)
			[[ -d "$PROC_CWD" ]] && NVIM_CWD="$PROC_CWD"
		fi
		[[ -z "$NVIM_CWD" ]] && NVIM_CWD="$HOME"
	fi

	# Check if aoe window already exists with same CWD - just show it
	AOE_WINDOW=$(hyprctl clients -j 2>/dev/null | jq -r '.[] | select(.class == "aoe-cwd") | .class' 2>/dev/null)
	CURRENT_AOE_CWD=""
	if [[ -f /tmp/aoe_cwd_current ]]; then
		CURRENT_AOE_CWD=$(cat /tmp/aoe_cwd_current 2>/dev/null)
	fi

	if [[ -n "$AOE_WINDOW" && "$CURRENT_AOE_CWD" == "$NVIM_CWD" ]]; then
		pypr show aoe
		exit 0
	fi
